...
| indicatorPlacement | `after`      | The position of the active tab indicator. Placement follows the `orientation` flow: `before` renders at the start edge (top or left), `after` at the end edge (bottom or right). |
